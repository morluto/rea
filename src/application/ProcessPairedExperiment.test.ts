...
const cloneCapture = (
  capture: ReturnType<typeof parseProcessCapture>,
): ReturnType<typeof parseProcessCapture> =>
  parseProcessCapture(structuredClone(capture));
  ...
  it("runs same-side comparisons first and compares stable sides", async () => {
    ...
    const capturePort: ProcessCapturePort = {
      capture: (scenario) => {
        ...
        return Promise.resolve(ok(cloneCapture(capture)));
      },
    };
    ...
    expect(result.value.cross_side_blocked_reason).toBeNull();
  });
...
describe("paired process experiment comparisons", () => {
  it("distinguishes normalized timing variance from semantic output drift", () => {
    const base = parseProcessCapture(EMPTY_PROCESS_CAPTURE_EXAMPLE);
    const early = parseProcessCapture({
      ...base,
      frames: [{ sequence: 0, at_ms: 10, data: "ready" }],
    });
    const late = parseProcessCapture({
      ...base,
    ...
    const changed = parseProcessCapture({
      ...base,
  ...
  it("blocks full cross-side output when only a subset was repeatable", async () => {
    const capture = parseProcessCapture(EMPTY_PROCESS_CAPTURE_EXAMPLE);
    const capturePort: ProcessCapturePort = {
      capture: () => Promise.resolve(ok(cloneCapture(capture))),
    };
