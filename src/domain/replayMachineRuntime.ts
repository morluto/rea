...
export class ReplayMachineRuntime {
  ...
  readonly #sensitiveVariables = new Set<string>();
  ...
  redact(value: string): string {
    let redacted = value;
    for (const variable of [...this.#sensitiveVariables].sort()) {
      const secret = this.#variables.get(variable);
  ...
  dispatch(event: ReplayMachineEvent): ReplayMachineDecision {
    if (!this.#admitEvent(event)) return this.#refusal("limit_exhausted");
    ...
    if (eligible.length === 0)
      return this.#refusal(
        matching.length === 0
          ? "unmatched"
          : event.protocol === "websocket_connect" &&
              event.connection === "reconnect"
            ? "unexpected_reconnect"
            : "invalid_state",
      );
    const guarded = eligible.find(({ transition }) =>
    ...
    if (guarded === undefined) return this.#refusal("guard_failed");
    const transition = guarded.transition;
    const used = this.#uses.get(transition.id) ?? 0;
    if (
      used >= transition.max_uses ||
      this.#timeline.length >= this.machine.max_transitions
    )
    ...
    const captures = transition.captures.map((capture) => ({
      ...capture,
      captured: readValue(capture.value, event, transition),
    }));
    if (captures.some(({ captured }) => captured === undefined))
      return this.#refusal("invalid_capture");
    for (const capture of captures) {
      this.#variables.set(capture.variable, capture.captured);
      if (capture.sensitive) this.#sensitiveVariables.add(capture.variable);
    }
    const record = {
