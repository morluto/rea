...
| Apple 애플리케이션   |   1 | 인증된 IPA 인벤토리를 애플리케이션 루트, 구성 요소, 런타임 계열 및 명시적 브리지 가설로 투영                                                                                    |
| Android 애플리케이션 |   1 | 인증된 APK 인벤토리를 구성 요소, 런타임 계열 및 명시적 managed-to-native 브리지 가설로 투영                                                                                     |
| 런타임 식별          |   1 | 실행 없이 런타임 계열을 식별하고 정확한 아티팩트 관찰과 의미 분석 공급자 가용성을 명시적으로 보고                                                                               |
...
| 애플리케이션         |   7 | 제한된 교차 계층 추적, 고유 일치 버전 비교, 승인된 Linux 격리 extracted-module replay, 관리형 런타임 특성화, 재구성 커버리지 폐쇄                                               |
