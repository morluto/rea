...
try {
  ...
  try {
    ...
    if ((await client.listTools()).tools.length !== 43)
      throw new Error("packaged MCP did not expose 43 tools");
  ...
  process.stdout.write(
    `${JSON.stringify({ cli: true, incurMcpCommand: "npx -y @morluto/rea mcp", doctor: true, setup: "idempotent", clients: 2, backupReadback: true, failureRecovery: true, skill: true, mcpTools: 43, targetFree: true })}\n`,
  );
} finally {
