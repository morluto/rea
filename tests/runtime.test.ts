...
describe("production stdio runtime", () => {
  it("starts the built entrypoint, lists 88 tools, calls one, and shuts down", async () => {
    ...
    try {
      ...
      expect(tools.tools).toHaveLength(88);
  ...
  it("honors the configured kind for an initial database target", async () => {
    ...
    try {
      await client.connect(transport);
      expect((await client.listTools()).tools).toHaveLength(88);
    } finally {
      await client.close();
      await transport.close();
    }
