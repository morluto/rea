...
describe("tool contract inventory", () => {
  it("publishes 71 analysis contracts and eighteen session tools", () => {
    ...
    expect(TOOL_CONTRACTS).toHaveLength(89);
    expect(new Set(TOOL_CONTRACTS.map(({ name }) => name)).size).toBe(89);
  });
