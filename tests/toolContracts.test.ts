...
describe("tool contract inventory", () => {
  it("publishes 60 analysis contracts and eighteen session tools", () => {
    ...
    expect(TOOL_CONTRACTS).toHaveLength(78);
    expect(new Set(TOOL_CONTRACTS.map(({ name }) => name)).size).toBe(78);
  });
