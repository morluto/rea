...
describe("tool contract inventory", () => {
  it("publishes 72 analysis contracts and eighteen session tools", () => {
    ...
    expect(TOOL_CONTRACTS).toHaveLength(90);
    expect(new Set(TOOL_CONTRACTS.map(({ name }) => name)).size).toBe(90);
  });
