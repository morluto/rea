...
describe("tool contract inventory", () => {
  it("publishes 66 analysis contracts and eighteen session tools", () => {
    ...
    expect(TOOL_CONTRACTS).toHaveLength(84);
    expect(new Set(TOOL_CONTRACTS.map(({ name }) => name)).size).toBe(84);
  });
