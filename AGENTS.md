...
- `src/contracts/`: caller-visible schemas for 33 direct, 10 enhanced, 5 native, 2 artifact, 3 managed, 8 browser, 4 Electron, 3 application, and 18 session tools; `enhancedInputs.ts` owns enhanced input parsing.
...
- `npm run verify:package`: pack and test the CLI, setup transaction, skill, and 86-tool target-free MCP server in an isolated environment.
